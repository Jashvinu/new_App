...
def get_gndvi_params():
    gndvi_params = {
        'min': -0.5,  # Adjust as needed
        'max': 0.8,   # Adjust as needed
...
def calculate_and_display_map(poi, start_year, start_month, start_day, end_year, end_month, end_day, index_choice="GNDVI"):
    ...
    if index_choice == "NDNI":
        ndni = calculate_gndvi(image)
        ndni_params = get_gndvi_params() 
    else: 
   ...
   # ... (Printing min/max might be less relevant for NDNI)
    ...
    Map.addLayer(ndni.updateMask(maskImage), ndni_params, 'GNDVI image') 
